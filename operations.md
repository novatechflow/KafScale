...
|--------|-------------|-----------------|
| `kafscale_s3_health_state` | Current S3 health: 0=healthy, 1=degraded, 2=unavailable | > 0 |
| `kafscale_s3_latency_ms_avg` | Average S3 operation latency | > 500ms |
| `kafscale_s3_error_rate` | S3 error rate (errors/sec) | > 0.01 |

**Throughput**

| Metric | Description |
|--------|-------------|
| `kafscale_produce_rps` | Produce requests per second |
| `kafscale_produce_bytes_total` | Total bytes produced |
| `kafscale_fetch_rps` | Fetch requests per second |
| `kafscale_fetch_bytes_total` | Total bytes fetched |

**etcd snapshots**

| Metric | Description | Alert threshold |
|--------|-------------|-----------------|
| `kafscale_operator_etcd_snapshot_age_seconds` | Age of last successful snapshot | > 3600 (1 hour) |
| `kafscale_operator_etcd_snapshot_success_total` | Total successful snapshots | — |
| `kafscale_operator_etcd_snapshot_failure_total` | Total failed snapshots | Any increase |

**Consumer groups**

| Metric | Description |
|--------|-------------|
| `kafscale_consumer_group_lag` | Consumer group lag by topic/partition |
| `kafscale_consumer_group_members` | Active members per group |
...
The operator automatically manages etcd snapshots when configured:

```yaml
# In KafscaleCluster CRD
spec:
  etcd:
    snapshotEnabled: true
    snapshotInterval: 1h
    snapshotRetention: 24
    snapshotBucket: s3://my-bucket/kafscale-etcd-snapshots
```

Or via environment variables:

```bash
KAFSCALE_OPERATOR_ETCD_SNAPSHOT_ENABLED=true
KAFSCALE_OPERATOR_ETCD_SNAPSHOT_INTERVAL=1h
KAFSCALE_OPERATOR_ETCD_SNAPSHOT_BUCKET=s3://my-bucket/kafscale-etcd-snapshots
KAFSCALE_OPERATOR_ETCD_SNAPSHOT_RETENTION=24
```
